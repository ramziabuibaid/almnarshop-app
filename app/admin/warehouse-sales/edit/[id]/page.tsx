        ...
        (data.Items || []).map(async (item: any) => {
          ...
          if (item.DetailsID) {
            try {
              ...
              serialNos = await getSerialNumbersByDetailId(item.DetailsID, 'warehouse_sales');
              console.log('[EditWarehouseSalesInvoicePage] Loaded serial numbers from serial_numbers table for detail', item.DetailsID, ':', serialNos);
              ...
              if (serialNos.length === 0 && serialNosMap.has(item.DetailsID)) {
                ...
                console.log('[EditWarehouseSalesInvoicePage] Loaded serial numbers from details table (fallback) for detail', item.DetailsID, ':', serialNos);
              }
            } catch (err) {
              console.error('[EditWarehouseSalesInvoicePage] Failed to load serial numbers:', err);
              // Fallback to details table
              if (serialNosMap.has(item.DetailsID)) {
                serialNos = serialNosMap.get(item.DetailsID) || [];
                console.log('[EditWarehouseSalesInvoicePage] Using fallback serial numbers from details table:', serialNos);
              }
            }
          }
      ...
      <div className="space-y-6 font-cairo" dir="rtl">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100 font-cairo">تعديل فاتورة مبيعات المخزن</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm sm:text-base font-cairo">#{invoice?.InvoiceID}</p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div>
            ...
            <div className="mb-4">
              <button
                ...
                className="flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-900 dark:bg-slate-700 text-white rounded-lg hover:bg-gray-800 dark:hover:bg-slate-600 transition-colors font-cairo text-sm sm:text-base w-full sm:w-auto"
              >
                <Plus size={20} />
          ...
          {/* Actions */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-4 border-t border-gray-200 dark:border-slate-700">
            {canAccountant && (
              <button
