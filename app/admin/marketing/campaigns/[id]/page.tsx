...
import { updateCampaign, getCampaignWithProducts, getProducts, Product } from '@/lib/api';
...
export default function EditCampaignPage() {
  ...
  useLayoutEffect(() => {
    document.title = 'تعديل عرض ترويجي - Edit Campaign';
  }, []);
  ...
  const loadCampaign = async () => {
    try {
      ...
      setStartDate(formatDateForInput(new Date(campaign.start_date)));
      setEndDate(formatDateForInput(new Date(campaign.end_date)));

      // Load selected products
      if (campaign.products && campaign.products.length > 0) {
        // First, we need to fetch full product details since API returns basic structure
        const productIds = campaign.products
          .map((cp: any) => cp.product_id || cp.products?.product_id)
          .filter(Boolean);
        ...
        if (productIds.length > 0) {
          ...
          const products: CampaignProduct[] = campaign.products
            .map((cp: any) => {
              const productId = cp.product_id || cp.products?.product_id;
              const fullProduct = allProducts.find(
                (p) => (p.id || p.ProductID) === productId
              );
              return {
                product_id: productId,
                offer_price: cp.offer_price || 0,
                product: fullProduct || undefined,
              };
            })
            .filter((cp: CampaignProduct) => cp.product); // Only include products that were found
          setSelectedProducts(products);
        }
      }
    } catch (error) {
      console.error('[EditCampaignPage] Error loading campaign:', error);
      alert('فشل في تحميل العرض');
  ...
  const loadProducts = async () => {
    try {
      ...
      console.error('[EditCampaignPage] Error loading products:', error);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[EditCampaignPage] Error updating campaign:', error);
      alert(`فشل في تحديث العرض: ${error.message}`);
    } finally {
      setSaving(false);
    }
  };
