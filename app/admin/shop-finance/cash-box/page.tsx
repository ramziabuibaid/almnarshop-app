...
export default function ShopCashBoxPage() {
  ...
  const loadCashFlow = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getShopCashFlow();
      console.log('[ShopCashBox] Raw data from API:', data);
      ...
      const transformed: CashFlowTransaction[] = data.map((item: any) => {
        ...
        const checkAmount = parseFloat(item.cheque_amount || item.check_amount || 0);
        ...
        return {
          ...
          amount: totalAmount || fallbackAmount,
          notes: item.notes || '',
      ...
      console.error('[ShopCashBox] Failed to load cash flow:', err);
  ...
  const transactionsWithBalance = useMemo(() => {
    ...
    // الأحدث أولاً في الجدول
    return withBalance.sort((a, b) => {
      // First sort by date (newest date first - الأحدث أولاً)
      const dateA = new Date(a.date || 0).getTime();
      const dateB = new Date(b.date || 0).getTime();
      if (dateA !== dateB) return dateB - dateA; // DESC - newest date first
      
      // If same date, sort by created_at (newest time first - الأحدث أولاً)
  ...
  const handleDelete = async (tx: TransactionWithBalance) => {
    ...
    try {
      if (tx.receipt_id) {
        await deleteShopReceipt(tx.receipt_id);
      } else if (tx.payment_id) {
        await deleteShopPayment(tx.payment_id);
      }
      loadCashFlow();
    } catch (err: any) {
  ...
  const handleReceiptSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Reload data in background without blocking - use longer delay to allow UI to settle
      setTimeout(() => {
        loadCashFlow();
      }, 500);
      ...
      console.error('[ShopCashBox] Failed to save receipt:', err);
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[ShopCashBox] Failed to save payment:', err);
    ...
    <AdminLayout>
      <div className="space-y-6" dir="rtl">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">صندوق المحل</h1>
            <p className="text-gray-600 mt-1">سجل مالي مستمر للمحل</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-medium font-cairo"
            >
              <Plus size={20} />
              سند قبض
            </button>
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-medium font-cairo"
            >
              <Plus size={20} />
              سند دفع
            </button>
                            ...
                            onClick={async () => {
                              ...
                              try {
                                ...
                                if (tx.receipt_id) {
                                  ...
                                  // getShopReceipt returns PascalCase fields (CustomerID, Date, CashAmount, ChequeAmount, Notes)
                                  ...
                                  // getShopPayment returns PascalCase fields (CustomerID, Date, CashAmount, ChequeAmount, Notes)
                                  const dateValue = transactionData.Date || transactionData.date || '';
                                  const formattedDate = dateValue 
                                    ? (typeof dateValue === 'string' && /^\d{4}-\d{2}-\d{2}/.test(dateValue) 
                                        ? dateValue.split('T')[0] 
